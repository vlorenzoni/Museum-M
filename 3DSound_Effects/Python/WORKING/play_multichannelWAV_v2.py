...
# Calculate durations and samples
dry_pan_duration = 24  # seconds
crossfade_duration = 5  # seconds
wet_duration = 30  # seconds
total_duration = dry_pan_duration + crossfade_duration + wet_duration

samples_dry = int(dry_pan_duration * fs)
samples_crossfade = int(crossfade_duration * fs)
samples_wet = int(wet_duration * fs)

# Create output array with the correct number of channels
...
def add_sync_markers(audio, fs, marker_duration=0.1, marker_amplitude=0.5):
    ...
    """
    # Define marker positions (in seconds)
    markers = {
        'dry_start': 0,
        'crossfade_start': 24,
        'wet_start': 29,
...
def audio_callback(outdata, frames, time, status):
    ...
    if current_frame < len(output):
        ...
        if current_time < dry_pan_duration:
            ...
            print(f"\r{phase}: {current_time:.1f}s / {dry_pan_duration:.1f}s ({progress:.0f}%)", end="", flush=True)
        elif current_time < dry_pan_duration + crossfade_duration:
            phase = "Crossfade"
            fade_progress = (current_time - dry_pan_duration) / crossfade_duration * 100
            print(f"\r{phase}: {current_time:.1f}s / {total_duration:.1f}s (Fade: {fade_progress:.0f}%)", end="", flush=True)
        else:
            phase = "Wet sound expansion"
            wet_time = current_time - (dry_pan_duration + crossfade_duration)
            expansion_stage = int(wet_time / 5) + 1  # Every 5 seconds
            print(f"\r{phase} (Stage {expansion_stage}): {current_time:.1f}s / {total_duration:.1f}s", end="", flush=True)
    else:
