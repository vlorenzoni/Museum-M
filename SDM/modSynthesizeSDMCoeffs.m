function [H,Hbin] = modSynthesizeSDMCoeffs(P,DOA, p,rot)
...
%%%%%%%%%%%%%%%%%%%%%%
%%% ROTATE PLOT by rotating AZ vector
%%%%%%%%%%%%%%%%%%%%%%
% shifted_data = az + rot * pi/180;
% % Wrap values to [-180, 180] range
% az = mod(shifted_data + pi, 2*pi) - pi;

% figure;
% plot(az);hold on

shifted_data = az.*180/pi + rot;

az = (mod(shifted_data + 180, 360) - 180) .*pi/180;
% plot(az); legend('original','rotated'); hold off


% shifted_data = AZ + rotation;
% AZ = mod(shifted_data + 180, 360) - 180;% Wrap values to [-180, 180] range
%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%
...
if p.Binaural
    ...
    N2 = size(p.hrir_l,1);
    nfft = N1+N2+1;
    %% multiplication of the H vector with the actual hrtfs from CIPIC
